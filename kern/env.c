...
int env_alloc(struct Env **new, u_int parent_id) {
    ...
    p_vars->pp_ref++;
    e->env_vars = (struct Var *)page2kva(p_vars);
	...
	// Lab 6 Shell Challenge: Initialize CWD and environment variables.
	if (parent_id == 0) {
		// This is a root process (e.g., 'init'), set CWD to "/"
		// The page for env_vars is already zeroed by page_alloc, so no further action needed.
		strcpy(e->env_cwd, "/");
	} else {
		// This is a child process, inherit CWD and environment variables.
		struct Env *parent_env;
		if (envid2env(parent_id, &parent_env, 0) == 0) {
			// Inherit CWD from the parent.
			strcpy(e->env_cwd, parent_env->env_cwd);
			
			// Inherit all exportable variables from parent.
			// The page for child's env_vars is zeroed, so we only need to copy.
			for (int i = 0; i < MAX_VARS; i++) {
				if ((parent_env->env_vars[i].flags & VAR_USED) &&
				    (parent_env->env_vars[i].flags & VAR_EXPORT)) {
					e->env_vars[i] = parent_env->env_vars[i];
				}
			}
		} else {
			// Fallback: If parent is not found, default to root CWD and no vars.
			strcpy(e->env_cwd, "/");
			printk("WARNING: env_alloc failed to get parent env %08x, defaulting CWD to / and no vars inherited\n", parent_id);
		}
	}
...
void env_free(struct Env *e) {
	...
	// FIX: Free the page allocated for environment variables
	if (e->env_vars) {
		page_decref(pa2page(PADDR(e->env_vars)));
		e->env_vars = NULL;
	}
