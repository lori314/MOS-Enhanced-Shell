...
int open(const char *path, int mode) {
	...
	if (resolve_path(path, resolved_path, sizeof(resolved_path)) < 0) {
		return -E_BAD_PATH;
	}
	...
	// Step 2: Prepare the 'fd' using 'fsipc_open' in fsipc.c with the modified mode.
	if ((r = fsipc_open(resolved_path, mode, fd)) < 0) {
		return r;
	}

	// Step 3: Set up mapping for the file's data.
	char *va = fd2data(fd);
...
int remove(const char *path) {
	// MODIFICATION: Resolve path before calling fsipc_remove.
	char resolved_path[MAXPATHLEN];
	if (resolve_path(path, resolved_path, sizeof(resolved_path)) < 0) {
		return -E_BAD_PATH;
	}

	// Call fsipc_remove with the resolved path.
	/* Exercise 5.13: Your code here. */
	return fsipc_remove(resolved_path);
}
