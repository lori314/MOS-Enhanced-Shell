...
/*
 * REFACTORED: Serve to open a file specified by the path in `rq`.
 * This version correctly handles O_CREAT and O_APPEND flags.
 */
void serve_open(u_int envid, struct Fsreq_open *rq) {
	...
	// If creating a file (or directory).
	if (rq->req_omode & O_CREAT) {
		r = file_create(rq->req_path, &f);
		if (r < 0) {
			// If file exists or any other creation error, fail immediately.
			ipc_send(envid, r, 0, 0);
			return;
		}
		// If creation was successful, mark it as a directory if requested.
		if (rq->req_omode & O_MKDIR) {
			f->f_type = FTYPE_DIR;
		}
	} else {
		// If not creating, just open an existing file.
	...
	// Sanity check: if O_MKDIR was passed, the result MUST be a directory.
	if ((rq->req_omode & O_MKDIR) && (f->f_type != FTYPE_DIR)) {
		ipc_send(envid, -E_NOT_A_DIR, 0, 0);
		return;
	}
	...
	// If mode includes O_TRUNC, set the file size to 0.
	...
	// NEW: If mode includes O_APPEND, set the file offset to the end of the file.
	// This is done by writing the current size into the Filefd struct, which the
	// client-side library will use as the initial offset.
	if (rq->req_omode & O_APPEND) {
		struct Filefd *temp_ff = (struct Filefd *)o->o_ff;
		temp_ff->f_fd.fd_offset = f->f_size;
	}


	// Fill out the Filefd structure to be sent to the client.
	...
	// Send the FileFd page to the caller.
...
int main(int argc, char **argv) {
	debugf("DEBUG: FS server process started. main() entered.\n"); // The only change
